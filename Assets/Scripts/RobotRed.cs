...
public class RobotRed : MonoBehaviour
{
    ...
    {
        cargoTracker.ResetP1();
        if (1 <= cargoList.Count)
        {
            if ((string)cargoList[0] == "Blue")
                cargoTracker.p1Blue1.enabled = true;
            else
                cargoTracker.p1Red1.enabled = true;

            if (2 <= cargoList.Count)
            {
                if ((string)cargoList[1] == "Blue")
                    cargoTracker.p1Blue2.enabled = true;
                else
                    cargoTracker.p1Red2.enabled = true;
            }
        }
    }
}
